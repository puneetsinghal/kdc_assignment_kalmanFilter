clear; clc;
addpath(genpath(fullfile(pwd,'p2n')));
figure;

for filesIndex = 0:9
    clear markersn state_estimate A C variance_estimate kalman_gain
    clc;
    fileName = strcat('p2n0', int2str(filesIndex));
    markersn = load(fullfile(pwd, 'p2n', fileName));
...
%     load markersn_ass4.mat
    ...
    %% Saving file
    fid = fopen(fullfile(pwd, 'results', strcat('p2a0', int2str(filesIndex), '.txt')), 'wt');

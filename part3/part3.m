clear;
addpath(genpath(fullfile(pwd,'p3n')));
figure;
% load marker data file
% load markersn_ass4.mat
for filesIndex = 0:9
    clc;
    fileName = strcat('p3n0', int2str(filesIndex));
    markersn = load(fullfile(pwd, 'p3n', fileName));
    ...
    for index = 1:length(markersn)
        ...
        clear visible_markers R marker_error C
        visible_markers = find(markersn(index,:)~=1e10);
        
        % Measurement noise model: marker variance
        R = 1*diag(ones(length(visible_markers), 1));
        
        
        % Compute measurement error
        [ marker_error, C ] = predict_markers(state_estimate, ...
            index, markersn, visible_markers);
        
        % Store values
        quaternions_before_measurement( index, : ) = state_estimate(7:10)';
        states_before_measurement( index, : ) = state_estimate';
        marker_errors( index, visible_markers ) = marker_error;
        for i = 1:n_x
    ...
    %% Saving file
    fid = fopen(fullfile(pwd, 'results', strcat('p3a0', int2str(filesIndex), '.txt')), 'wt');
    fprintf(fid, 'x\ty\tz\txd\tyd\tzd\tq0\tq1\tq2\tq3\twx\twy\twz\tm0x\tm0y\tm0z\tm1x\tm1y\tm1z\tm2x\tm2y\tm2z\tm3x\tm3y\tm3z\tm4x\tm4y\tm4z\tm5x\tm5y\tm5z\tm6x\tm6y\tm6z\tm7x\tm7y\tm7z\n');
    for i = 1:n_t
        fprintf(fid,'%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n',...
            ...
            markers_position(i,1), markers_position(i,2), markers_position(i,3), markers_position(i,4), markers_position(i,5), markers_position(i,6), ...
            markers_position(i,7), markers_position(i,8), markers_position(i,9), markers_position(i,10), markers_position(i,11), markers_position(i,12), ...
            markers_position(i,13), markers_position(i,14), markers_position(i,15), markers_position(i,16), markers_position(i,17), markers_position(i,18), ...
            markers_position(i,19), markers_position(i,20), markers_position(i,21), markers_position(i,22), markers_position(i,23), markers_position(i,24));
    end
    fclose(fid);
